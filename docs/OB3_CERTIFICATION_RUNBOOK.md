...
- `pnpm lint`
- `pnpm typecheck`
- `pnpm test -- apps/api-worker/src/http-smoke.test.ts apps/api-worker/src/ob3-oauth.test.ts apps/api-worker/src/credential-verification.test.ts apps/api-worker/src/public-badge-page.test.ts`
...
Optional strict proof expectation (for conformance rehearsal):

```bash
CERT_BASE_URL="https://<deployment-host>" \
CERT_BOOTSTRAP_ADMIN_TOKEN="<token>" \
CERT_JOB_PROCESSOR_TOKEN="<optional-token>" \
CERT_EXPECTED_PROOF_TYPE="DataIntegrityProof" \
CERT_EXPECTED_CRYPTOSUITE="eddsa-rdfc-2022" \
pnpm cert:preflight
```

Live mode will:

- Validate `/ims/ob/v3p0/discovery` metadata and OAuth registration/token URLs
...
- Process queue jobs
- Verify `GET /credentials/v1/:credentialId` returns active/valid checks for:
  - `jsonLdSafeMode`
  - `credentialSchema` (not invalid)
  - `credentialSubject`
  - `dates`
  - `credentialStatus`
  - `proof` (including optional expected proof type/cryptosuite)
- Verify public badge page retrieval and required verification/import UI signals
- Save evidence JSON at `artifacts/ob3-certification/preflight-*.json`
- Save a manual submission checklist at `artifacts/ob3-certification/preflight-*.md`
...
5. Complete and archive the generated checklist markdown.
...
Template for final evidence package:

- `docs/OB3_CERTIFICATION_EVIDENCE_TEMPLATE.md`
...
- Preflight checklist markdown(s)
...
The script automates engineering preflight and evidence generation. 1EdTech portal submission, reviewer communication, and final approval remain manual external steps.
